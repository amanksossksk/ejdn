    ...
    const menu = {
        reply_markup: {
            inline_keyboard: [
                ...
                [{ text: "📁 Bulk Check (TXT File)", callback_data: "bulk_check_file" }],
    ...
    try {
        ...
        if (action === "multi_thread_check") {
            ...
            safeSendMessage(chatId,
                ...
                userStates[userId] = { action: 'awaiting_multi_thread_file' };
            });
            return;
        }
            ...
            const adminMenu = {
                reply_markup: {
                    inline_keyboard: [
                        ...
                        [{ text: "🚫 Manage Users", callback_data: "admin_manage_users" }], // NEW
...
if (action === "admin_trial_settings") {
    ...
    return;
        ...
        if (action === "admin_view_subscriptions") {
            ...
            const activeSubscriptions = Object.entries(config.subscriptionExpiry);
            ...
            const settingsMenu = {
                reply_markup: {
                    inline_keyboard: [
                        ...
                        [{ text: "👑 Subscription Plans", callback_data: "admin_subscription_plans" }],
                        [{ text: "⚙️ Trial Settings", callback_data: "admin_trial_settings" }], // NEW
...
bot.on('document', async (msg) => {
    ...
    console.log(chalk.cyan(`📁 File received: ${msg.document.file_name} from user ${userId}`));
    ...
    if (!userState || !['awaiting_bulk_file', 'awaiting_multi_thread_file'].includes(action)) {
        return safeSendMessage(chatId,
            "⚠️ *No file expected!*\n\n" +
    ...
    try {
        ...
        await safeEditMessage(chatId, processingMsg.message_id,
            ...
            `• Mode: ${action === 'awaiting_multi_thread_file' ? '🚀 Multi-thread' : '📁 Single-thread'}\n\n` +
            `⏳ <b>Starting verification...</b>`
        );
        
        let results;
        try {
            if (action === 'awaiting_multi_thread_file') {
